...
FMT_CONSTEXPR auto invoke_parse(parse_context<Char>& ctx) -> const Char* {
  ...
  constexpr bool is_it_formattable =
      std::is_constructible<formatter<mapped_type, Char>>::value;
  if (!is_it_formattable) return ctx.begin();  // Error is reported in the value ctor.
  using formatted_type = conditional_t<is_it_formattable, mapped_type, int>;
